---
title: "apex-mcp Application Generator Tools Full Reference"
sidebarTitle: "Generators"
description: "Reference for apex-mcp high-level generators: CRUD pages, dashboards, login, reports, wizards, modal forms, schema-to-app, and REST endpoints."
---
...
<ParamField path="auth_scheme" type="string">Optional authorization scheme name to restrict access to this region.</ParamField>
...
<ParamField path="detail_sql" type="string" required>SQL for the detail IR. Must reference a bind variable matching `P{page_id}_{page_item_name}` in its WHERE clause.</ParamField>
<ParamField path="link_column" type="string" required>Column in the master IR whose value is passed to the hidden item when a row is clicked (e.g., `"ID"`).</ParamField>
<ParamField path="page_item_name" type="string" required>Suffix for the hidden page item that stores the selected master row value (auto-prefixed with `P{page_id}_`). The bind variable in `detail_sql` must match.</ParamField>
<ParamField path="sequence" type="integer" default="10">Display sequence for the master region (detail region gets `sequence + 10`).</ParamField>
...
<ParamField path="sequence" type="integer" default="10">Display sequence for the filter region (IR gets `sequence + 10`).</ParamField>
