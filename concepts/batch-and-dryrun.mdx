---
title: "Batch Mode and Dry-Run Mode in apex-mcp"
...
The `rollback_on_error` parameter (default: `True`) controls what happens if any statement in the batch fails:

- **`rollback_on_error=True` (default):** If any statement raises an Oracle error, the entire batch is rolled back. No partial changes are committed. The returned log will contain an `ERR:` line for the failing statement and a `ROLLBACK:` summary line.
- **`rollback_on_error=False`:** All statements that succeed are committed, even if others fail. The log will contain `ERR:` lines for failures and a `COMMIT: partial commit...` summary line.

Using the default `True` is recommended during app builds — a failed component creation that partially succeeds can leave your app in an inconsistent state.
